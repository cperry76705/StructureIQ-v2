...
class DashboardOverview:
    ...
    paper_auto_approval_mode: str = "manual"
    last_auto_approval_block_reason: str | None = None
    candidates_auto_approved: int = 0
    candidates_blocked_from_auto_approval: int = 0
...
class ResearchDashboardService:
    ...
    def overview(self) -> DashboardOverview:
        ...
        return DashboardOverview(
            ...
            auto_approval_enabled=bool(getattr(getattr(continuous_status, "config", None), "auto_approve_candidates", False)),
            manual_approval_required=not bool(getattr(getattr(continuous_status, "config", None), "auto_approve_candidates", False)),
            max_trades_per_cycle=int(getattr(getattr(continuous_status, "config", None), "max_trades_per_cycle", 1) or 1),
            max_candidates_per_cycle=int(getattr(getattr(continuous_status, "config", None), "max_candidates_per_cycle", 3) or 3),
            paper_auto_approval_mode=("controlled_auto" if getattr(getattr(continuous_status, "config", None), "auto_approve_candidates", False) else "manual"),
            last_auto_approval_block_reason=getattr(orchestrator_status, "last_auto_approval_block_reason", None),
            candidates_auto_approved=int(getattr(orchestrator_status, "total_candidates_approved", 0) or 0),
            candidates_blocked_from_auto_approval=int(getattr(orchestrator_status, "total_candidates_blocked_from_auto_approval", 0) or 0),
