...
def test_version_command_prints_current_version(capsys) -> None:
    ...
    assert exit_code == 0
    assert "StructureIQ v6.0.17" in output
...
def test_market_paper_order_requires_explicit_opt_in() -> None:
    args = start.parse_args(["--paper", "--order-type", "market"])
    with pytest.raises(ValueError, match="allow-market-orders"):
        start.build_paper_start_payload(args)
...
def test_logging_writes_startup_log(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "version=6.0.7" in contents
