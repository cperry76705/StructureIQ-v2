...
Continuous runtime report generation is non-overwriting. A cycle reports `generated`, `skipped_existing`, `disabled`, or `failed`; an existing report adds `daily_report_skipped_existing` to cycle blocking information without turning the cycle into an error. Runtime status and sessions expose separate generated and skipped-existing totals. The manual `/reports/daily/generate` endpoint retains explicit `overwrite=true` support.
